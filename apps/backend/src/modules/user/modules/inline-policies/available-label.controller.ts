...
export class AvailableLabelController {
	...
	@ApiInternalServerErrorResponse({ description: 'If failed to get availability status of the label' })
	...
	public async availableLabel(
		...
		this.logger.log(
			`Will try to get availability status of label: "${label}" with a user ID: "${userId}"`,
		);
		...
		this.logger.log(
			`Successfully got availability status of label: "${label}" with a user ID: "${userId}"`,
		);
